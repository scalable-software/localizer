...
```
specifications/<component>.specifications.json     ← source of truth
        │
        ├─→ src/<component>.meta.ts            vocabulary + value-domain types
        ├─→ src/<component>.validation.ts      Validate class — runtime enforcement
        ├─→ src/<component>.ts                 state, operations, events, gestures
        ├─→ src/<component>.template.html      realized composition
        └─→ src/<component>.style.css          realized presentation
...
- **Test first.** Every behavioral change starts with a failing test and follows with a working implementation. Then a FAIL which is the tests are commited and after that the PASS which is the implementation is commited. No exceptions.
...
Two MCP servers are configured in `.mcp.json` for this project. Use them instead of the npm scripts they replace:

- **Wallaby (`wallaby`) — unit test status and coverage.** Wallaby runs the suite continuously in the IDE, so query it for pass/fail results, failing tests, and covered lines. Do not run `npm test` to check tests or coverage; `npm test` is the CI gate, not the development feedback loop. If Wallaby is not running, say so rather than falling back to `npm test`.
- **Chrome DevTools (`chrome-devtools`) — testing the demo.** Start the demo with `npm run serve`, then use the Chrome DevTools tools to open `demo/index.html`, read the console, inspect the DOM, and take screenshots. Do not verify demo behavior by reasoning about the code alone.

## Example Component

A complete compliant and working example: a pin component, is made available:

@examples/specifications/pin.specifications.json - Pin specifications
@examples/src/*.* - Pin Implementation
@examples/test/unit/*.* - Pin unit tests